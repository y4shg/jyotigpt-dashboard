...
# API Routes
@app.route('/api/jyotigpt/summary')
@login_required
def get_jyotigpt_summary():
    """Fetch JyotiGPT analytics summary"""
    try:
        jyotigpt_url = os.getenv('JYOTIGPT_URL')
        admin_token = os.getenv('JYOTIGPT_ADMIN_TOKEN')
        
        if not jyotigpt_url or not admin_token:
            return jsonify({'error': 'JyotiGPT credentials not configured'}), 400
        
        response = requests.get(
            f"{jyotigpt_url}/api/v1/analytics/summary",
        ...
        if response.status_code == 200:
            ...
            logger.error(f"JyotiGPT API error: {response.status_code}")
            return jsonify({'error': 'Failed to fetch JyotiGPT data'}), response.status_code
            
    except requests.exceptions.RequestException as e:
        logger.error(f"JyotiGPT request failed: {str(e)}")
        return jsonify({'error': str(e)}), 500

@app.route('/api/jyotigpt/models')
@login_required
def get_jyotigpt_models():
    """Fetch JyotiGPT model usage data"""
    try:
        jyotigpt_url = os.getenv('JYOTIGPT_URL')
        admin_token = os.getenv('JYOTIGPT_ADMIN_TOKEN')
        ...
        response = requests.get(
            f"{jyotigpt_url}/api/v1/analytics/models",
            headers={'Authorization': f'Bearer {admin_token}'},
            timeout=10
        )
        ...
        logger.error(f"JyotiGPT models request failed: {str(e)}")
        return jsonify({'error': str(e)}), 500

@app.route('/api/jyotigpt/users')
@login_required
def get_jyotigpt_users():
    """Fetch JyotiGPT user activity data"""
    try:
        jyotigpt_url = os.getenv('JYOTIGPT_URL')
        admin_token = os.getenv('JYOTIGPT_ADMIN_TOKEN')
        ...
        response = requests.get(
            f"{jyotigpt_url}/api/v1/analytics/users",
            headers={'Authorization': f'Bearer {admin_token}'},
            timeout=10
        )
        ...
        logger.error(f"JyotiGPT users request failed: {str(e)}")
        return jsonify({'error': str(e)}), 500
